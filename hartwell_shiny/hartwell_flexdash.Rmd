---
title: 'Housing Conditions: Hartwell survey results'
runtime: shiny
output:
...
#Some properties entered twice. Duplicates or multiple structures?
...
####################
sample<-read_csv("Data/parcels_hartwell_own.csv") %>%
  mutate(parcel_num=Parcel_No) %>%
  select(parcel_num,X,Y) %>%
  filter(X<0)

sample_points<-st_as_sf(sample,coords=c("X","Y"),crs=4326) 

# parcel_correct<-read_csv("commerce_parcelcorrection.csv")
...
parcel_points1<-parcel_points %>%
  select(reviewed_by,date,parcel_num,address,
...
The map on the right shows data already collected. Red dots are surveyed properties, and grey dots are those yet to be surveyed. The orange lines show ward boundaries. Mouse over them to see the ward numbers.
...
downloadHandler(filename = function() {
     paste("surveydata_hartwell_",Sys.Date(),".csv",sep="")
   },
     content = function(file) {
     write.csv(parcel_points1, file, row.names = FALSE)
   }
)
...
output$map1<-renderLeaflet({
  leaflet(parcel_points) %>% 
    setView(lng = median(parcel_points$x), lat=median(parcel_points$y), zoom = 15) 
})
...
observe({
  ...
  leafletProxy('map1',data=parcel_points) %>%
    ...
    addCircles(data=parcel_points,radius=12,opacity=1,fillOpacity=0.9,color="#222222",fillColor=
                 ~factpal(cond_factor), weight=.5,layerId = parcel_points$X_id) %>%
...
observe({
  ...
  prop_id<-parcel_points %>% filter(X_id==input$map1_shape_click$id)
...
conditionalPanel(
      ...
      selectInput("issue_ext","Issue selection",
             choices=c("Repainting needed"="exterior.ext_repaint",
                "Cracked/minor dry rot"="exterior.ext_cracked",
                "Needs replacement"="exterior.ext_needs_replace",
                "Chimney needs repair"="exterior.ext_chimney",
                "Missing/no siding"="exterior.ext_nosiding"),
             selected="exterior.ext_repaint"))

conditionalPanel(
      condition = "input.issue_cat == 'Windows/doors'",
      selectInput("issue_wind","Issue selection",
                choices=c("Repainting needed"="windows_doors.window_repaint",
                "Cracked panes"="windows_doors.window_crackedpanes",
                "Replace 1-3 windows"="windows_doors.window_minreplace",
                "Replace >3 windows"="windows_doors.window_majreplace")))

conditionalPanel(
      condition = "input.issue_cat == 'Roof'",
      selectInput("issue_roof","Issue selection",
                choices=c("Gutters need repair"="roofing.roof_gutters",
                "Cracked/peeling shingles"="roofing.roof_shingles",
                "Partial re-roofing needed"="roofing.roof_reroof_part",
                "Total re-roofing needed"="roofing.roof_reroof_tot",
                "New roofing structure needed"="roofing.roof_newstructure")))

conditionalPanel(
      condition = "input.issue_cat == 'Lot'",
      selectInput("issue_lot","Issue selection",
                choices=c("Lawn overgrown/weeds"="lot_assess.lot_weeds",
                "Missing ground cover"="lot_assess.lot_missingcover",
                "Dead/hazardous trees"="lot_assess.lot_trees",
                "Inoperable vehicle in yard"="lot_assess.lot_inop_vehicle",
                "Major cleanup/junk in yard"="lot_assess.lot_junk",
                "Porch used as storage"="lot_assess.lot_porchstorage",
                "Graffiti on house/property"="lot_assess.lot_graffiti")))
...
conditionalPanel(
 ...
 radioButtons("bmap2", "Choose the base map",
  ...
  downloadButton('downloadData','Download Result Set')
  downloadHandler(filename = function() {
     paste("surveydata_hartwell_issuemap_",Sys.Date(),".csv",sep="")
    },
     content = function(file) {
...
filteredData <- reactive({
 ...
 ifelse(input$issue_cat == "Stairs",
         ...
         filtervar<-"lot_weeds"))))))
  parcel_points_issue %>% filter(var==filtervar)
  })
...
observe({
  ...
  prop_id1<-parcel_points %>% filter(X_id==input$map2_shape_click$id)
  output$text2 <- renderUI({
    HTML(prop_id1$textbox)
  })
 })
# 
...
You can click on any of the red colored points to see more information about the property. They grey dots show unsurveyed parcels. This map allows you to see the locations of the most distressed housing overall, which can be useful in planning redevlopment efforts.

The second tab, Common issues-map, allows you to see properties with each type of issue. You can select the issue by type (Roof, Stairs, etc.) and then specifically from the drop down menu. The red dots are properties with that specific issue. As with the first tab, you can click on a property for more information. This map allows you to see where properties with common issues are located.
